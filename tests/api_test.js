...
async function testLMArena() {
    ...
    console.log(`\n🤖 === LM ARENA TESTER (TIMED & OPTIMIZED) ===`);
    let result = { success: false, imageUrl: null, time: null };
    ...
    try {
        ...
        await page.goto(targetUrl, { waitUntil: 'networkidle0', timeout: 30000 }); // Reduced timeout
        ...
        try {
            ...
            // Human-like pause after typing (like thinking) (reduced)
            ...
            while (!submitClicked && submitAttempts < maxSubmitAttempts) {
                ...
                try {
                    ...
                    if (submitButton) {
                        ...
                        // Wait for modal to appear (reduced)
            ...
            // Now check for Terms of Use modal that appears after submission
             ...
             try {
                 const modalAgain = await page.waitForSelector('div[role="dialog"]', { timeout: 5000 }); // Reduced
                 if (modalAgain) {
                     ...
                     while (!agreeClickedAgain && attemptsAgain < maxAttemptsAgain) {
                         ...
                         try {
                             ...
                             if (!agreeButtonAgain) {
                                 // Try to find button with "Agree" text and click using evaluate
                                 foundAgain = await page.evaluate(() => {
                                     const buttons = Array.from(document.querySelectorAll('button'));
                                     const agreeBtn = buttons.find(btn => btn.textContent.trim() === 'Agree');
                                     if (agreeBtn) {
                                         agreeBtn.click();
...
// Run the test
testLMArena().then(result => {
    console.log('\n📊 Test Summary:');
    console.log('- Model selected: ideogram-v3-quality');
    console.log('- Prompt used: A beautiful sunset over mountains with vibrant colors');
    console.log('- Attempts made for submit/modal handling');
    console.log('- Image generation monitored for up to 60 checks');
